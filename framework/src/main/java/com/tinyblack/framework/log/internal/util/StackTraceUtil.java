...
package com.tinyblack.framework.log.internal.util;
...
public class StackTraceUtil {
  ...
  private static final String TLOG_STACK_TRACE_ORIGIN;

  static {
    // Let's start from xlog library.
    String xlogClassName = TLog.class.getName();
    TLOG_STACK_TRACE_ORIGIN = xlogClassName.substring(0, xlogClassName.lastIndexOf('.') + 1);
  }
  ...
  private static StackTraceElement[] getRealStackTrack(StackTraceElement[] stackTrace,
    ...
    for (int i = allDepth - 1; i >= 0; i--) {
      ...
      if (className.startsWith(TLOG_STACK_TRACE_ORIGIN)
