...
import com.tinyblack.framework.log.internal.SystemCompat;
 ...
 * <p>
 * A {@link Logger} is always generated and mostly accessed by {@link TLog}, but for customization
 * purpose, you can configure a {@link Logger} via the {@link Builder} which is returned by
 * {@link TLog} when you trying to start a customization using {@link TLog#tag(String)}
 ...
 * the {@link Logger} assuming that you are using the {@link TLog} directly.
 */
public class Logger {
    ...
    /*package*/ void setLogConfiguration(LogConfiguration logConfiguration){
        this.logConfiguration = logConfiguration;
    }
    /**
    ...
    private void printlnInternal(int logLevel, String msg) {
        ...
        if (msg.contains("{") && msg.trim().length()>1 && msg.lastIndexOf("{")!=msg.length()-1) {
            msg = msg.replace("{", "\n{\n");
        }
        if (msg.contains("}") && msg.length()>1&& msg.lastIndexOf("}")!=msg.length()-1&& msg.lastIndexOf("},")!=msg.length()-2) {
            msg = msg.replace("}", "\n}\n");
        }
        if (msg.contains("[")&& msg.lastIndexOf("[")!=msg.length()-1) {
            msg = msg.replace("[", "[\n\t ");
        }
        if (msg.contains("]")&& msg.length()>1&& msg.lastIndexOf("]")!=msg.length()-1&& msg.lastIndexOf("],")!=msg.length()-2) {
            msg = msg.replace("]", "\n]");
