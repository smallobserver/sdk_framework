...
//    static void setStatusBarColorForCollapsingToolbar(final Activity activity_play, final AppBarLayout appBarLayout, final CollapsingToolbarLayout collapsingToolbarLayout,
...
//            int statusBarHeight = getStatusBarHeight(activity_play);
...
//        //移除已经存在假状态栏则,并且取消它的Margin间距
//        int statusBarHeight = getStatusBarHeight(activity_play);
//        removeFakeStatusBarViewIfExist(activity_play);
//        removeMarginTopOfContentChild(mContentChild, statusBarHeight);
//        //添加一个View来作为状态栏的填充
//        final View statusView = addFakeStatusBarView(activity_play, statusColor, statusBarHeight);
//
...
//    }
//
//    static void setStatusBarWhiteForCollapsingToolbar(final Activity activity_play, AppBarLayout appBarLayout, final CollapsingToolbarLayout collapsingToolbarLayout, Toolbar toolbar, final int statusBarColor) {
//        final Window window = activity_play.getWindow();
//        window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
//
//        ViewGroup mContentView = (ViewGroup) window.findViewById(Window.ID_ANDROID_CONTENT);
//        View mContentChild = mContentView.getChildAt(0);
...
//        try {
//            Class<?> layoutParams = Class.forName("android.mView.MiuiWindowManager$LayoutParams");
...
//            e.printStackTrace();
//        }
//        final View statusView = addFakeStatusBarView(activity_play, color, statusBarHeight);
//        CoordinatorLayout.Behavior behavior = ((CoordinatorLayout.LayoutParams) appBarLayout.getLayoutParams()).getBehavior();
//        if (behavior != null && behavior instanceof AppBarLayout.Behavior) {
//            int verticalOffset = ((AppBarLayout.Behavior) behavior).getTopAndBottomOffset();
//            if (Math.abs(verticalOffset) > appBarLayout.getHeight() - collapsingToolbarLayout.getScrimVisibleHeightTrigger()) {
...
//            @Override
//            public void onOffsetChanged(AppBarLayout appBarLayout, int verticalOffset) {
//                if (Math.abs(verticalOffset) >= (appBarLayout.getTotalScrollRange() - Eyes.getPxFromDp(activity_play, 56))) {
//                    if (appBarLayoutState != COLLAPSED) {
//                        appBarLayoutState = COLLAPSED;
//
//                        if (Eyes.MIUISetStatusBarLightMode(activity_play, true) || Eyes.FlymeSetStatusBarLightMode(activity_play, true)) {
//                        }
//                        if (statusView.getAlpha() == 0) {
//                            statusView.animate().cancel();
//                            statusView.animate().alpha(1f).setDuration(collapsingToolbarLayout.getScrimAnimationDuration()).start();
...
//
//                        if (Eyes.MIUISetStatusBarLightMode(activity_play, false) || Eyes.FlymeSetStatusBarLightMode(activity_play, false)) {
//                        }
//                        if (statusView.getAlpha() == 1) {
//                            statusView.animate().cancel();
//                            statusView.animate().alpha(0f).setDuration(collapsingToolbarLayout.getScrimAnimationDuration()).start();
//                        }
//                        translucentStatusBar(activity_play);
//                    }
//                }
//            }
//        });
